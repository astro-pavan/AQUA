...
*   Astronomy and Astrophysics, 621, A128 (2019)
...
************************************************************************
* Version 18.08.21. Compared with the originally published version,
* a CORRECTION WAS MADE, which affects the entropy and thus 
* the free energy (FNkT), but not the other outputs of H2OFIT.
************************************************************************
*                           MAIN program
...
C%C      write(*,112)
C%C   10 continue
C%C      write(*,'('' Input RHO [ 0 to stop]: ''$)')
C%C      read*,RHO
C%C      if (RHO.le.0.) stop
C%C      call H2OFIT(RHO,T,PnkT,FNkT,UNkT,CV,CHIT,CHIR,PMbar,USPEC)
C%C      SNk=UNkT-FNkT
C%C      write(*,111) RHO,T,PMbar*100,PnkT,FNkT,UNkT,CV,CHIT,CHIR,SNk
C%C      goto 10
C%C      stop
C%C  112 format('   rho[g/cc]    T [K]      P [GPa]   P/(n_i kT)',
C%C     *  ' F/(N_i kT) U/(N_i kT) C_V/(N_i k)  chi_T     chi_r',
C%C     *  '      S/(N_i k)')
C%C  111 format(1P,4E12.4,11E11.3)
C%C      end
...
* 7. Second (low-T) thermal correction:
      TTC2=TTC**2
      ULC1=1.d0+TTC2
      FC=(PC1*dlog(ULC1/TTC2)+PC2*datan(TTC))/TCRIT-PC3/TEMP ! 18.08.21
